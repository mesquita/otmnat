...
class Population(object):
    ...
    def mutation(self, tp_mut='binary', mutation_rate=0.2):
        if tp_mut == 'binary':
            for idx, parent in enumerate(self.mating_pool):
                for i, allel in enumerate(parent.gen):
                    if np.random.uniform(0, 1) < mutation_rate:
                        self.mating_pool[idx].gen[i] = int(not (allel))
        ...
        if tp_mut == 'float':
            for idx, parent in enumerate(self.mating_pool):
                ...
                parent.sigma = np.array(parent.sigma)
                parent.gen = np.array(parent.gen)

                x_candidato = parent.gen + parent.sigma * local_uni
                x_candidato = list(x_candidato)
                ...
                if (indv_candidato.score < parent.score):
                    self.mating_pool[idx] = indv_candidato
        ...
        if tp_mut == 'metaEP':
            for idx, parent in enumerate(self.mating_pool):
                global_n = np.random.normal(loc=0.0, scale=1.0)
                local_n = np.random.normal(loc=0.0, scale=1.0, size=self.num_alelo)
                alpha = 0.2
